...
def init(self: object = None,
    ...
    processes = [Process(
        ...
        args=(multi_env_cls, env_creator, env_args, env_kwargs,
              ...
              step_shared_mem[i], reset_shared_mem[i],
    ...
    return namespace(self,
        ...
        prev_env_id = [],
        env_pool = env_pool,
        reset_shared_mem=reset_shared_mem,
        step_shared_mem=step_shared_mem,
...
def _worker_process(multi_env_cls, env_creator, env_args, env_kwargs,
    ...
    while True:
        ...
        # TODO: Handle put/get
        if request in [b'step', b'reset', b'resetNone']:
...
def send(state, actions):
    ...
    for i, atns in zip(state.prev_env_id, actions):
        state.step_shared_mem[i][:] = atns.tobytes()
        state.send_pipes[i].send_bytes(b"step")

def async_reset(state, seed=None):
    reset_precheck(state)
    if seed is None:
        for pipe in state.send_pipes:
            pipe.send_bytes(b"resetNone")
    else:
        for idx, pipe in enumerate(state.send_pipes):
            state.reset_shared_mem[idx][0] = seed + idx
            pipe.send_bytes(b"reset")
